...
class Client:
    ...
    d = None
    try:
        with open(args.root_dir + '/data/behavior_trace/user_behavior_trace.json', 'r', encoding='utf-8') as f:
            d = json.load(f)
            print('Loaded user behvaiour is successful')
    ...
    def __init__(self, client_id, group=None, train_data={'x' : [],'y' : []}, eval_data={'x' : [],'y' : []}, model=None, device=None, cfg=None):
        ...
        if self.device.get_device_model() == None:
            if self.cfg.hard_hete:
                self.device.set_device_model(real_device_model)
            else:
                self.device.set_device_model("Redmi Note 8")

    # Ahmed - KDN average models
    def average_with_model(self, model):
        a_norm = norm_grad(model)
        b_norm = norm_grad(self.model.get_params())
        logger.info("BEFORE id:%s server norm %f, client norm %f", self.id, a_norm, b_norm)
        copy_param = copy.deepcopy(self.model.get_params())

        for i, v in enumerate(copy_param):
            if norm_grad(model[i].astype(np.float64)) != norm_grad(copy_param[i].astype(np.float64)):
                #copy_param = copy.deepcopy(self.model.get_params()[i].astype(np.float64))
                #self.model.get_params()[i] = tf.reduce_mean(self.model.get_params()[i], model[i])
                copy_param[i] = (1.0 * copy_param[i] + 1.0 * model[i]) / 2.0
                #self.model.get_params()[i] = np.mean(np.array(self.model.get_params()[i].astype(np.float64), model[i].astype(np.float64)), axis=0)
                #logger.info("PARAM Update old %f new %f updated %f", norm_grad(copy_param), norm_grad(model[i].astype(np.float64)), norm_grad(self.model.get_params()[i].astype(np.float64)))
                #self.model.get_params()[i] = (0.3 * self.model.get_params()[i].astype(np.float64) + 0.7 * model[i].astype(np.float64)) #(1.0 * v.astype(np.float64) + 1.0 * model[i].astype(np.float64)) / 2.0
        self.model.set_params(copy_param)
        b_norm = norm_grad(self.model.get_params())
        logger.info("AFTER id:%s server norm %f, client norm %f", self.id, a_norm, b_norm)
        del copy_param
    ...
    def train(self, start_t=None, num_epochs=1, batch_size=10, minibatch=None):
        ...
        def train_with_simulate_time(self, start_t, num_epochs=1, batch_size=10, minibatch=None):
            ...
            if (down_end_time-start_t) > self.deadline:
                ...
                # Ahmed - a hack to fix the error related to failed clients computation - (IndexError: index -9223372036854775808 is out of bounds for axis 0 with size 11)
                if self.cfg.hard_hete and not self.cfg.behav_hete:
                    available_time = self.deadline * self.cfg.num_rounds
                else:
                    available_time = self.timer.get_available_time(start_t + self.act_download_time, train_time_limit)
                comp = max(0.0, self.model.get_comp(data, num_epochs, batch_size))
                self.actual_comp = max(0.0, int(comp*available_time/train_time) )  # will be used in get_actual_comp

                #Ahmed - Debug comp info for failed client - (IndexError: index -9223372036854775808 is out of bounds for axis 0 with size 11)
                logger.debug(len(data['y']), num_epochs, batch_size, comp, available_time, train_time, self.actual_comp)
                ...
                self.actual_comp = max(0.0, self.model.get_comp(data, num_epochs, batch_size))
                if self.cfg.fedprox:
                    ne = -1
                    for i in range(1, num_epochs):
                        et = self.timer.get_future_time(down_end_time, train_time*ne/num_epochs + upload_time)
                ...
                if minibatch is None:
                    ...
                    num_epochs = 1
                    if self.cfg.no_training:
                        comp = max(0.0, self.model.get_comp(data, num_epochs, num_data))
                        update, acc, loss, grad, loss_old = -1,-1,-1,-1,-1
                    else:
                        comp, update, acc, loss, grad, loss_old = self.model.train(data, num_epochs, batch_size)
    ...
    #Ahmed - KDN define a train function on the local dataset
    def train_local(self, num_epochs=1, batch_size=10, minibatch=None):
        if minibatch is None:
            num_data = min(len(self.train_data["x"]), self.cfg.max_sample)
        else:
            frac = min(1.0, minibatch)
        ...
        return comp, update, grad, acc, loss, loss_old, num_train_samples
    ...
    def set_deadline(self, deadline = -1):
        # Ahmed - enforce deadline for device heter as well
        #if deadline < 0 or not (self.cfg.behav_hete or self.cfg.hard_hete or self.cfg.uniform_hete):
        if deadline < 0 or not self.cfg.hard_hete:
    ...
    def get_actual_comp(self):
        ...
        '''
        #Ahmed - Change to resolve the bug related to np.histogram - not working
        return min(sys.maxsize, max(0.0, self.actual_comp))
