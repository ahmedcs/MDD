...
class Timer:
    ...
    try:
        update_cnt = 0
        with open('timer/cached_timers.json','r', encoding='utf-8') as f:
    ...
    def save_cache():
        with open('timer/cached_timers.json','w') as f:
    ...
    def __init__(self, ubt, google=True):
        ...
        self.guid = ubt['guid']
        self.time_zone = int(ubt['kb_time_zone'])
        ...
        for mes in message:
            try:
                ...
                # Ahmed - get the min and max among all timers
                if Timer.min_ref_sec == 0:
                    Timer.min_ref_sec = self.trace_start
                else:
                    Timer.min_ref_sec = min(self.trace_start, Timer.min_ref_sec)
                if Timer.max_ref_sec == 0:
                    Timer.max_ref_sec = self.trace_end
                else:
                    Timer.max_ref_sec = max(self.trace_end, Timer.max_ref_sec)
